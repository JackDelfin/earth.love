...
#
# StartTimer
#   - Sets the internal StartTime to now
#
#*****************************************************************************************
package Orbit;
...
sub Orbit::ShowStats
{
  ...
  #***************************************
  # Show Number of Tokens in memory
  my $cnt = keys %{$self->{_TOKENS}};
  $self->AddStat("_Tokens Memory", $cnt );

  #
  # Show the OML Function Groups Loaded
  #
  $cnt = keys %{$self->{_FN_GROUPS_LOADED}};
  $self->AddStat("_Function Groups Total", $cnt);
  if ($cnt>0) {
    foreach my $grp (sort(keys %{$self->{_FN_GROUPS_LOADED}})) {
      $self->AddStatUnique("_Function Group: ", $self->{_FN_GROUPS_LOADED}->{$grp});
    }
  }
  #***************************************
  # Orbit Customizations
