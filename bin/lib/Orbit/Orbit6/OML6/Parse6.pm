...
#*****************************************************************************************
#*
#*  Package Name    :   Orbit::OML6::Parse6
#*
...
#use Orbit::Utils;
#use Orbit::Token;
use Orbit::Orbit6::OML6::Function6;
use Orbit::Orbit6::OML6::Token6;
...
#******************************************************************************************
#*  Procedure Name  :   Parse <Buffer> [Print default:1]
#*
#*  Description     :   Parses Buffer, replacing out TOKENS and OML FUNCTIONS for their value
...
sub Orbit::Parse
{
  ...
  my @FunArgs       = { };             # Array of FUNction Arguments from the Params string
  my $FunctionValue = "";              # Return Function Value Buffer
  my $bProcessed    = 0;               # Return code from Process_Function
  ...
  while ($s != -1 && $e != -1 && $gb_stop == 0) {
    ...
    if ($Function ne "") {
      #
      # Set the Function Arguments $FunArgs from the Params text
      #
      @FunArgs = $self->setFunctionArgs($Params);

      #
      # Process the Function, passing in Function, Token, and arguments array.
      # Some functions use token name: ASSIGN/BACK
      #
      ($FunctionValue, $bProcessed) = $self->Process_Function($Function, $Token, $bFlush, @FunArgs);

      # Release memory from the Function Args
      undef @FunArgs;
    ...
    if ($Token ne "") {
      ...
      #
      if (!$self->Token_Exists($TokenName)) {
        # Doesn't exist, so create it to show it was referenced
        $self->Set_Token($TokenName, "");
      }
      #
      # Process Token Modifiers
      #
      $TokenValue = $self->Process_Modifiers($Token);
...
#******************************************************************************************
#*  Procedure Name  :   getNextSegment
#*
#*  Description     :   Gets the next token segment from the Output
...
sub Orbit::getNextSegment
{
  ...
  my $RecursiveCount = 0;
  my $RecursiveMax = $self->{_RecursiveMax}*7;
  ...
  while ($s != -1)
  {
    ...
    if ( $fas >= 0      # ]# fn Start and fn End exist
      ...
      if ($lFunction ne "" && $self->isOMLFunction($lFunction)) {
        ...
        #
        $lParams = $self->Fix_Arguments($lParams);
        ...
        if ($lParams =~ /#[A-Za-z][A-Za-z0-9_=.]*[ \t\n]*\[/) {
          ...
          while ($ns != -1 && $ne != -1) {
            ...
            $self->AddStat("_Function Calls Nested") if ($self->GetLogStat());
...
#******************************************************************************************
#*  Procedure Name  :   Fix_Arguments
#*
#*  Scope           :   PUBLIC
#*
#*  Description     :   Removes any whitespace between function parameters - space, tab, "\n", chr(13)
#*                      between ] and [
#*                      ie. #FUNCTION [x]  [y]  [z]#
#*                      Returns the cleaned argument String
#******************************************************************************************/
sub Orbit::Fix_Arguments
{
  my ( $self, $fv_list ) = @_;
...
#******************************************************************************************
#*  Procedure Name  :   setFunctionArgs
#*
#*  Scope           :   PUBLIC
#*
#*  Description     :   Accepts a delimited list of parameters to an OML Function ][
...
#******************************************************************************************
sub Orbit::setFunctionArgs
  #          fv_list                        IN OUT NOCOPY LONG
{
  my ( $self, $Args ) = @_;
  ...
  # Unset the array if nothing entered
  if (!defined($Args)) {
    return @FunArgs;
  }
...
#******************************************************************************************
#*  Procedure Name  :   EscapeOMLFunction
#*
#*  Scope           :   PUBLIC
#*
#*  Description     :   Escape the function argument separator '][' and function/assignment end ']#'
#*                      ][  =>  \]\[       ]#  =>  \]\#
#******************************************************************************************
...
#******************************************************************************************
#*  Procedure Name  :   UnEscapeOMLFunction
#*
#*  Scope           :   PUBLIC
#*
#*  Description     :   UnEscape the function argument separator '][' and function/assignment end ']#'
#******************************************************************************************
...
#******************************************************************************************
#*  Procedure Name  :   StripComments
#*
#*  Scope           :   PUBLIC
#*
#*  Description     :   Remove comments <!--- ---> from the text
#******************************************************************************************
sub Orbit::StripComments
{
  ...
  $Text = $self->Strip_Text($Text, '<!---', '--->', '', 1, $bShowComments);
...
#******************************************************************************************
#*  Procedure Name  :   Strip_Text
#*
#*  Scope           :   PUBLIC
#*
#*  Description     :   Iteratively strips text strings out of a given string based on the
...
sub Orbit::Strip_Text
{
  ...
  while (1)
  {
    ...
    if ($bKeepRawText && $fv_replace_text eq "") {
      ...
      #$fv_replace_text = $U->Get_Raw_Text($fv_replace_text);
...
#print "\n[".substr($fv_text, 0, $li_start)."]\n";
#print "\n[".$fv_replace_text."]\n";
#print "\n[".substr($fv_text, 0, $li_start).$fv_replace_text."]\n";
    ...
    #BEFORE
    #$fv_text = substr($fv_text, 0, $li_start)
    #          .$fv_replace_text
    #          .substr($fv_text, $li_end + $li_end_length);
  }
...
# use Orbit::OML6::Parse6;
#******************************************************************************************
1;


#END Orbit::OML6::Parse6 Package
