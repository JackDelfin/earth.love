...
#*****************************************************************************************
#*
#* Orbit::OML::Function::SysAdmin - Version 7
#*
#*  Package Name    :   Orbit::OML:Function::SysAdmin
#*
#*  Description     :   Implements Orbit Markup Language (OML) for SYSADMIN Functions
#*
#*****************************************************************************************
# History:
...
#*****************************************************************************************
# LoadSysAdminFunctions
#   - Load the SysAdmin OML Functions into the _OML_FUNCTIONS subroutine reference array
#
# DISTANCE
# BEARING
#*****************************************************************************************
...
#   - Load the SysAdmin OML Functions into the _OML_FUNCTIONS subroutine reference array
#
sub Orbit::LoadSysAdminFunctions
{
  my ( $O ) = @_;
  $O->{_OML_FUNCTIONS}->{'CPUPROC'}    = sub { $O->omlCPUPROC(), };
  $O->{_OML_FUNCTIONS}->{'MEMORY'}     = sub { $O->omlMEMORY(), };
  $O->{_OML_FUNCTIONS}->{'DISK'}       = sub { $O->omlDISK(), };
  $O->{_OML_FUNCTIONS}->{'SITES'}      = sub { $O->omlSITES(), };
  $O->{_OML_FUNCTIONS}->{'BLOCKEDIP'}  = sub { $O->omlBLOCKEDIP(), };
} #LoadSysAdminFunctions
# Alias - Function Group SysAdmin 12
*LoadFunctionGroup12 = \&LoadSysAdminFunctions;
...
#END Orbit::OML:Function::SysAdmin Package
