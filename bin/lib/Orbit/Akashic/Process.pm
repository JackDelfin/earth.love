...
#
# RefreshPageData
#     ["<ROOT>*"]   ["<word/phrase/path>*"]   ["<datafile(s)>*"]   [DaysBack]   [LogLevel]
#
...
#  NOTES:
#*******************************************************************************
#  - The PAGE displayed is set by the SetStaticPage() function, usual in Orbit::myConfig.pm.
#    Defaults to EL_SHOW_STATIC
#  - SetStaticHost sets the default hostname, default http://localhost
#  - SetStaticContext sets the Context Prefix, defining cgi-bin location, default /cgi-bin/
#  - The OBJECT Token is set to DATA_<Datafile>, which EL_SHOW_STATIC references
#    as EL_SHOW_DATA_<Datafile>.  This is the "handler" for each Datafile type.
#  - The following tokens are set prior to showing the page:
#    ROOT, DATA, WORD
#    _STATIC_ - set to 1 to be used for Boolean logic within pages
#
...
sub Orbit::RefreshPageData {
  ...
  $O->SetShowOutput(1); # Turn on ShowOutput - Show processing messages
...
#  if (!open(SOURCE, '<', $ROOTS)) {   # UTF-8 Fix
  ...
  $O->SetToken('_PAGES_', $O->GetStat('__Pages Total'));
  ...
  sub Orbit::FindPageFiles
  {
    ...
    if (-T
      ...
      # Refresh the Page
      #
      # Set _STATIC_ file generation flag so pages use DOTDOT and DATAPATH
      $O->Set_Token('_STATIC_', 1);
      # Set the HOST and Context Prefix (cgi-bin) for Batch Processing so dynamic links still work
      $O->Set_Token('ENV_HOST',           $self->{_STATIC_HOST});      # http://localhost
      $O->Set_Token('ENV_CONTEXT_PREFIX', $self->{_STATIC_CONTEXT});   # /o/ - cgi-bin directory
      # Get the Static Page to show for Batch Processing
      $O->Set_Token('PAGE',               $self->{_STATIC_PAGE});      # EL_SHOW_STATIC
      # Set other context data
      $O->Set_Token('ROOT', $O->GetRoot());
      $O->Set_Token('DATA', $data);
      $O->Set_Token('WORD', $word);
      $O->Set_Token('OBJECT', 'DATA_'.$data);
      
      # Set the DOTDOT Token for the Root-Word
      # Only if we are in the WORD/PHRASE/PATH directory
      if ($filename eq $A->{_Data}->{'_WORD'}
        ||$filename eq $A->{_Data}->{'_PHRASE'}
        ||$filename eq $A->{_Data}->{'_PATH'}
        ) {
        # Fully qualified WPP
        $O->SetDotDotWPP($O->GetRoot(), $word);
      } else {
        # Partial WPP directory: INDEX, DOWN
        my $rootdir = $dir;
        # Get the root dir without Domain for Relative DotDot path
        $rootdir = substr($rootdir, length($O->{_DomainDir}));
        $O->SetDotDot($rootdir);
      }
      ...
      $O->SetPrintOutputOff();
      #
      my $OUT = $O->ShowPage('EL_SHOW_STATIC');

      # Get the output file: datafile.html
      my $dataLower = $data;
      $dataLower =~ tr/[A-Z]/[a-z]/;   # lowercase datafile for .html output
      my $PageOut = "";
      # Use index.html for WORD PHRASE PATH datafiles
      if ($filename eq $A->{_Data}->{'_WORD'}
        ||$filename eq $A->{_Data}->{'_PHRASE'}
        ||$filename eq $A->{_Data}->{'_PATH'}
        ) {
        $PageOut = $dir.'index.html';
      } else {
        # else use datafile name for html file
        $PageOut = $dir.$dataLower.'.html';
      }
