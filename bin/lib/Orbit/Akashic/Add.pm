...
#*******************************************************************************
# History:
#   2021.07.14 earth.love oK Added Image Upload functionality
...
#**************************************************************************
  ...
  if      ($DataType eq 'CAT') {
    ...
    $Tags     = 'FILE MULTI UNIQUE NOWHO';
$self->AppendError("name:$name  Filename:$Filename Tags:$Tags\n");   #TESTING
...
#$O->Append_Token('_DEBUG_', "$ff:($value) ");   # if ($ff eq 'desc');
      ...
      #
      $O->AppendError($U->initcap($datafile)." #MSG[is required]#");
  ...
  if ($bProcessData) {
    ...
    #
    if (!$A->AddData($root, $name, $DataType, $Filename, $WHO, $Data, $Tags )) {
      ...
      # Error
      #
      $O->AppendError('#MSG[Error adding Data:]# '.$A->GetUprintBuf());
